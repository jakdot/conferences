"""
Model replicating long-time learning.
"""
...
addition = actr.ACTRModel(subsymbolic=True, retrieval_threshold=-2.25, latency_factor=0.1, instantaneous_noise = 0.25, decay=DECAY, buffer_spreading_activation={"g": 1}, strength_of_association=3.3, association_only_from_chunks=False)
...
TIME = 10*86400 #time span in which training occurs

UPTO = 6 #upto which number is counted

SIMULATION = 500 #number of steps in each simulation cycle
...
results, _, init_time, ul = simulation2(1, SIMULATION, init_time, results, False, {}, feedback=True, learning=True)
...
for x in range(1,8):
    results = {}
    results, _, init_time, ul = simulation2(SIMULATION*x, SIMULATION*(x+1), init_time, results, False, {}, feedback=True, learning=True, ul=ul)
    record(results, x+1)

results = {}
results, time_stamps, init_time, ul = simulation2(SIMULATION*4, SIMULATION*6, init_time, results, True, {}, feedback=False, learning=True, ul=ul)
